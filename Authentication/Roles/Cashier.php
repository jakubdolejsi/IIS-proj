...
class Cashier extends RegisteredUser
{
	...
	public function getAllTickets()
	{
		$query = 'select t.id, cw.name, ce.begin, ce.date, t.price, t.seat, h.label, t.payment_type, t.is_paid from xdolej09.ticket as t 
				...
				join xdolej09.hall as h on ce.id_hall = h.id order by ce.date asc, ce.begin asc';
		$date = date('Y-m-d');
	...
	public function newReservation($params, $data): void
	{
		...
		$createUserQuery = 'insert into xdolej09.user (firstName, lastName, email, password, role, hash, is_verified) 
						VALUES (?,?,?,?,?,?,?)';
		$x = $this->db->run($createUserQuery, $userValues);
		$id = $this->db->lastInsertId();
		$priceQuery = 'select price from xdolej09.culture_event where id = ?';
