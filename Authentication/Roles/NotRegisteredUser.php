...
class NotRegisteredUser extends Password
{
	...
	public function completeRegistration(): bool
	{
		...
		$query = 'UPDATE xdolej09.user SET firstName=?, lastName=?, password=? where email=?';
	...
	public function completeVerification()
	{
		...
		$query = 'UPDATE xdolej09.user SET role=?, is_verified=? where id=?';
	...
	{
		$data = $this->loadPOST();
		$query = 'select * from xdolej09.user where email=?';
	...
	public function getReservedSeatInfo($params)
	{
		...
		$queryHall = 'select hall.row_count, hall.column_count from xdolej09.hall join culture_event ce on hall.id = ce.id_hall
		...
		$queryReservedSeats = 'select ticket.seat from xdolej09.ticket 
	...
	{
		$id = $_SESSION['user_id'];
		$query = 'select * from xdolej09.user where id=?';
	...
	public function insertHash($hashCode)
	{
		...
		$query = 'UPDATE xdolej09.user SET user.hash=? where user.email=?';
	...
	public function oneTimeRegister(): void
	{
		...
		if (empty($user)) {    //Neni potreba vkladat zaznam
			$query = 'INSERT INTO xdolej09.user(email, role) VALUES (?, ?)';
	...
	public function register(): void
	{
		...
		$this->processRegistrationPassword($userDetail);
		$query = 'INSERT INTO xdolej09.user(firstName, lastName, email, password, role) VALUES (?, ?, ?, ?, ?)';
	...
	public function verifiedUser(): void
	{
		...
		$query = 'update xdolej09.user set is_verified = 1 where user.email = ?';
	...
	public function verifyHash()
	{
		...
		$query = 'select user.hash from xdolej09.user where id=? and hash=?';
	...
	{
		$existingReservationQuery = 'select * from xdolej09.ticket as t 
