...
class SearchModel extends BaseModel
{
	...
	{
		$date = date('Y-m-d');
		$query = 'select ce.id, cw.name, cw.type, cw.genre, ce.date, ce.begin, h.label, ce.price from xdolej09.culture_event as ce 
				join xdolej09.culture_work as cw on ce.id_culture_work = cw.id
				join xdolej09.hall as h on ce.id_hall = h.id where ce.date >= ? order by ce.date asc, ce.begin asc';
